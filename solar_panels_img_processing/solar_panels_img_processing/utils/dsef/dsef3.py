...
import solar_panels_img_processing.utils.dsef.dsef3_tools as df3t

class Dsef:
    def __init__(self, im, edge_direction, dir_span=90, t_crit=None, dtype=np.float64, force_dtheta=None):        
        self.dtype=dtype
        # Initialize the directional step edge filter bank
        self.DF                = df3t.DsefFilters(edge_direction=edge_direction, dir_span=dir_span, force_dtheta=force_dtheta)
        ...
        print("pads",self.upad,self.vpad)
        ...
        self.ep2D              = df3t.gen_epanechnikov2D_kernel(self.DF.radius)
        ...
        PoiVar                 = 150.0
        dof_edge               = df3t.calc_dof(PoiVar, PoiVar, self.Neff)
        dof_end                = df3t.calc_dof(PoiVar, PoiVar, self.Neff)
        CRIT_FAC=1#
        self.crit_edge         = t_crit if t_crit is not None else CRIT_FAC * df3t.get_t_critical(df=dof_edge)
        self.crit_end          = t_crit if t_crit is not None else CRIT_FAC * df3t.get_t_critical(df=dof_end)
        
    def _within_bounds(self, u, v):
        """
    ...
    def find_best_direction(self):
        ...
        t = [df3t.dsef_test(self, self.u, self.v, direction, edge_direction = 0, FORWARD=True).FORWARD for direction in sel_dirs]    
        # Find best direction to follow
        ind_max = np.argmax(t)
        if (t[ind_max] < t[len(sel_dirs)//2] + self.crit_edge):  # Stay on previous track
            ind_max = len(sel_dirs)//2
            print("Stay on previous track")
        return sel_dirs[ind_max], sel_dirvecs[ind_max]
     
    def EdgeSearch(self, start, stop, img, OPTIMIZE_SEARCH_DIRECTION=False):
        """
        Search for edge. 
        ...
        while v_heading[0]*v[0] + v_heading[1]*v[1] > 0:       # until we have passed the stop point
            ...
            c,r    = self.get_pos()
            T_FULL = df3t.dsef_test(self, self.u, self.v, self.edge_direction, FULL=True).FULL
    ...
    def EdgeFollow(self, MAX_ITT = 1000):
        """
        ...
        #Fabio's change to start follow in the best direction from EdgeSearch instead of guess direction
        t1 = [df3t.dsef_test(self, self.u, self.v, direction, FORWARD=True, FULL=True).FULL for direction in sel_dirs]    
        ind_max = np.argmax(t1)
        #print("t original search",t1)
        best_direction = sel_dirvecs[ind_max]
        # initialize rewa with best direction from the EdgeSearch
        ...
        while Nitt < MAX_ITT:
            ...
            t = [df3t.dsef_test(self, self.u, self.v, direction, FORWARD=True, FULL=True).FULL for direction in sel_dirs]    
            ...
            T_FORWARD = t[ind_max]
            print("new direction:",linetools.calc_heading(v))
            T_ALL = df3t.dsef_test(self, self.u, self.v, sel_dirs[ind_max], ALL=True).ALL            
            ALL_EDGE    = np.all(np.array(T_ALL) > self.crit_edge)

            # FORWARD_EDGE = T_FORWARD > self.crit_edge

            # Pixel edge test. Fabio:why do we add 1 to consec_edge when ALL_EDGE?
            if ALL_EDGE:
            ...
            if consec_edge >= Ntest_edge:
                ...
                print("var_direction",var_direction)
            ...
            print("MOVE DIR:",linetools.calc_heading(v))
        ...
        return EDGE_FOUND, END_FOUND, REWA, message, us, vs
