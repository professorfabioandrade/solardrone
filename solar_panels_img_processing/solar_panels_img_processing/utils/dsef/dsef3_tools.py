...
def calc_heading_vector(heading_deg, dtype=np.float64):
    """
...
def gen_epanechnikov2D_kernel(r2, INDEX=False):
    # Generate the kernel
...
class FlutDir:
    ...
    """
    def __init__(self, d_theta, center = 0):
        self.dir_span = 360
        self.center   = center
    ...
    def set_span(self, center, dir_span):
        """
    ...
    def get_nearest(self, direction):        
        """
        INPUTS:
...
class DsefFilters:
    ...
    """
    def __init__(self, edge_direction, radius=15, N=3, bu=0, bv=0, dir_span=90, force_dtheta=None):
        ...
        #v1          = bv+(2*self.N-1)*radius         # filter center top    why (2*self.N-1)? 
        v1 = bv + radius   
        self.R = math.hypot(self.bu + self.radius, self.bv + self.radius)
        print("KERNEL RADIUS:",self.R)
        ...
        print("d_theta",d_theta)   #11.25 degrees for radius 15 and N 3 and span 90
    ...
    def set_direction(self, edge_direction, dir_span):
        """
    ...
    def _calc_regs(self, heading_deg, dtype=int):
        """
...
def nw(c, r, kernel, im_pad, mask_pad, dtype=np.float64):
    """
...
def nw_calc(E, rcL, rcR, dtype=np.float64):
    """
...
def dsef_test(E, u, v, direction, ALL = False, FULL=False, FORWARD=False, REAR=False, END=False, USE_LUT=True):
    """
